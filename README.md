...
* 12.2 xdog视觉伺服以及语音控制模块
  ...
  - **控制逻辑**：
  	- **语音控制**：感知（ESP32） → 理解（ASR + LLM） → 决策（本地控制） → 执行（ROS）![语音控制](files/images/第十二节/12-2-语音控制.png)
  	- **目标跟踪**：感知（RealSense） → 决策（YOLO + Kalman Tracking + 误差计算） → 执行（PID）![目标跟踪](files/images/第十二节/12-2-目标跟踪.png)
  - **实战演练**：
  	- **语音控制**：
